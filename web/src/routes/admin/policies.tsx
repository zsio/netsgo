    ...
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="rounded-xl border border-border/40 bg-card/50 backdrop-blur-sm p-6 shadow-sm overflow-hidden">
        {isLoading || !policy ? (
          ...
          <AdminPoliciesForm key={JSON.stringify(policy)} initialPolicy={policy} />
        )}
      </div>
    </div>
  );
...
function AdminPoliciesForm({ initialPolicy }: { initialPolicy: TunnelPolicy }) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const ports = blockedPortsStr
      .split(',')
      .map((value) => Number.parseInt(value.trim(), 10))
      .filter((value) => !Number.isNaN(value));
    const hosts = whitelistStr
      .split(',')
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="flex flex-col gap-2">
          ...
          <Input
            ...
            onChange={(e) => setFormState({ ...formState, min_port: Number.parseInt(e.target.value, 10) || 0 })}
        ...
        <div className="flex flex-col gap-2">
          ...
          <Input
            ...
            onChange={(e) => setFormState({ ...formState, max_port: Number.parseInt(e.target.value, 10) || 0 })}
