    ...
    <>
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          {/* Dialog */}
          <div className="relative bg-card border border-border rounded-xl p-6 shadow-lg max-w-lg w-full mx-4 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-start gap-3 mb-4">
              <div className="shrink-0 w-10 h-10 rounded-full bg-amber-500/10 flex items-center justify-center">
                <AlertTriangle className="w-5 h-5 text-amber-500" />
              </div>
              <div>
                <h3 className="text-lg font-semibold">端口白名单变更影响提示</h3>
            ...
            {/* 受影响的隧道列表 */}
            <div className="max-h-60 overflow-y-auto rounded-lg border border-border/60 bg-muted/20 mb-5">
                ...
                <tbody>
                  {affectedTunnels.map((t, i) => (
                    <tr key={i} className="border-b border-border/20 last:border-0">
                      <td className="py-2 px-3 font-mono text-xs">{t.hostname || t.client_id.slice(0, 8)}</td>
