...
- 当前实际接口以 `/api/clients` 为准，不要想当然写成 `/api/agents`。
- `go test ./...` 在没有 `web/dist` 时失败，通常不是 Go 逻辑坏了，而是 embed 前置条件没满足。
