...
- **探针监控** — 实时采集 Client 所在机器的 CPU、内存、磁盘、网络状态
...
> 当前版本说明：`client --server` 参数目前仍需传入 `ws://host:port` 形式的 WebSocket 基地址。
> 规划中的 TLS/WSS 目标态会将地址模型统一为公共 `http://` / `https://` 基地址，并在内部推导控制/数据通道地址，详见 `P1解决方案一_设计文档.md` 与 `docs/implementation-plan.md`。
...
> 说明：当前实现中的数据通道仍为同端口二进制协议（`PeekListener + 0x4E + yamux`）。
> `/ws/data` WebSocket 化属于正在设计和评审中的目标架构，详见 `P1解决方案一_设计文档.md`。
