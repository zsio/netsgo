...
2. 当前运行二进制版本号是否能获取？否 → 提示开发构建不支持自动更新，退出
...
```
GitHub:   https://github.com/zsio/netsgo/releases/download/v{version}/netsgo_{version}_{os}_{arch}.tar.gz
ghproxy:  https://ghproxy.com/https://github.com/zsio/netsgo/releases/download/v{version}/netsgo_{version}_{os}_{arch}.tar.gz
```

其中 `{version}` 为去掉前导 `v` 的版本号；例如 tag `v1.2.3` 对应 asset `netsgo_1.2.3_linux_amd64.tar.gz`。ARMv7 资产名遵循 `.goreleaser.yaml`，即 `netsgo_1.2.3_linux_armv7.tar.gz`。
...
| 有新版本 | 先展示当前/最新版本与下载通道，再明确确认是否下载、替换并重启 |
| 选定通道请求失败 | 显示 `Update failed` 摘要与具体错误，本次更新结束；用户可重新运行并改选通道，或手动下载后使用 `netsgo upgrade` |
| 无法解析版本号 | 提示无法比较版本，退出 |

### 3.6 下载与替换流程

1. 从选定通道获取最新版本号
2. 若发现新版本，先让用户明确确认是否继续下载、替换并重启
3. 下载对应平台的 release asset（`.tar.gz`）
4. 下载同一 release 的 `checksums.txt`，按 GoReleaser 默认 `sha256` 与 `<hexhash><two spaces><filename>` 格式校验归档
5. 解压获取二进制（接受正常 tar 路径变化，但仅提取安全相对路径中的常规文件 `netsgo`，忽略目录、符号链接和其他 entry type）
6. **停止所有已安装的受管服务**（server 和/或 client）
7. 替换 `/usr/local/bin/netsgo`
8. **启动所有已安装的受管服务**
9. 输出升级结果

### 3.7 文案

- 菜单项：`update   - 检查并更新到最新版本`
- 可更新摘要：`Update available`
- 用户取消摘要：`Update cancelled`
- 失败摘要：`Update failed`（附具体错误）
...
4. 版本比较：
   ...
   b. 已安装版本无法确定 → 明确警告无法完成版本安全检查，默认取消，用户确认后继续（`--force` 可跳过）
   c. 当前 < 已安装 → 提示降级风险，默认取消，用户确认后继续
   d. 当前 == 已安装 → 提示"已是最新版本，无需升级"，退出
   e. 当前 > 已安装 → 正常继续
```
...
- 已安装版本未知提示：`无法确定已安装版本，版本安全检查无法完成。是否继续？默认取消`
...
只操作已安装的角色。通过 `svcmgr` 的固定 service layout、unit/env 文件、binary、角色 runtime dir、server runtime DB（仅 server 强制要求）和 systemd 状态判断。
...
**固定做法：** 直接执行 `/usr/local/bin/netsgo --version` 获取，不依赖本地 service manifest 字段。
...
| 错误场景 | 行为 |
|---------|------|
| 下载失败（网络/超时） | 提示具体错误并结束本次更新流程；用户可重新运行并改选通道，或手动下载后使用 `upgrade` |
| 下载成功但校验失败 | 拒绝替换，提示重新下载 |
| 替换二进制后服务无法启动 | 自动回滚到本次更新前的旧版本，并恢复先前停止的服务；若回滚本身失败，则明确提示手动修复 |
...
3. **下载后必须校验 release 归档与同版本 `checksums.txt` 的 sha256**，校验失败拒绝替换
4. **只从安全 tar 相对路径中提取常规文件 `netsgo`**，避免路径穿越、符号链接替换或误提取其他文件
5. **不自动降级**（除非用户明确确认）
6. **dev 版本必须用户确认**，防止开发误操作
7. **已安装版本未知时必须用户确认**，不能静默跳过保护
8. **不写入敏感信息**到任何文件
9. **使用临时目录下载和解压**，完成后清理
...
| `internal/manage/update.go`（新增） | `update` 的核心逻辑：版本检查、确认、下载、替换、重启 |
| `cmd/netsgo/cmd_upgrade.go`（新增） | `upgrade` 命令的 CLI 入口 |
| `pkg/updater/update.go` / `pkg/updater/upgrade.go` | `update` / `upgrade` 的核心逻辑：版本检查、替换、重启与失败回滚 |

### 8.3 不修改的内容

- 不改变 `cmd/netsgo/cmd_update.go` 的结构（它仍作为 `rootCmd` 的独立命令注册，`manage` 的 `update` 通过 `internal/manage` 实现）
- 不修改 `.goreleaser.yaml` 或 Release 流程
- 不新增 CI 上传步骤
