...
- 受管服务的 spec / env / journald 查看统一按 root 管理，避免普通用户获得额外查看面
...
- 若发现 `<data-dir>/server/` 中存在有效且完整的 server 数据：
  ...
  - 若用户确认沿用：只重新写 unit / spec / env 并启动服务
...
- `/etc/netsgo/services/server.json`：`root:root`，`0600`
- `/etc/netsgo/services/server.env`：`root:root`，`0600`
- `/etc/netsgo/services/client.json`：`root:root`，`0600`
- `/etc/netsgo/services/client.env`：`root:root`，`0600`

说明：

- 即使 `*.json` 主要记录非敏感安装信息，本期也统一按 root-only 处理，避免普通用户直接读取受管服务元数据

## 8.3 install spec / env
...
- `*.json` 用于保存安装规格，例如角色、service 名称、binary 路径、data dir、unit path、运行用户等
...
- **已安装**：对应 role 的 unit 与 spec/env 存在
- **未安装但存在历史数据**：无 unit/spec/env，但对应角色 data dir 下的数据存在
- **损坏安装**：unit、spec/env、binary、data dir 下的数据组合不一致
...
- install 不得把管理员明文密码持久化到 `server.env` 或 `server.json`
...
5. 写入 `/etc/netsgo/services/server.json`
6. 写入 `/etc/netsgo/services/server.env`
7. 安装 `/etc/systemd/system/netsgo-server.service`
...
4. 写入 `/etc/netsgo/services/client.json`
5. 写入 `/etc/netsgo/services/client.env`
6. 安装 `/etc/systemd/system/netsgo-client.service`
7. `systemctl daemon-reload`
8. `systemctl enable --now netsgo-client.service`
...
- 安装后 systemd unit、spec、env、data dir 都在正确路径
