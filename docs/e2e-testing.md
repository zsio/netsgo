...
Do not add Docker cross-version coverage for the removed client-scoped tunnel mutation API. Current and `v0.1.8` `netsgo client` do not expose `Client.ProxyConfigs` through CLI flags, environment variables, or a config file. The old `/api/clients/{id}/tunnels...` write surface has been removed; the product write surface is the unified `/api/tunnels` API. Do not add a test-only product switch just to manufacture a legacy mixed-version E2E.
