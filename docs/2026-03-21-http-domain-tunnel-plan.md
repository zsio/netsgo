...
| 文件 | 规划 |
|------|------|
| `internal/server/http_tunnel_rules.go`（建议新增） | 承载 HTTP 域名归一化、生效管理 Host 提取、runtime + store 共享声明扫描 helper、Host 命中判定、状态到返回码映射等“规则层”逻辑；避免把规则、I/O、反向代理全部塞进一个大文件。归一化（转小写、去尾部 `.`、去端口）由 API handler 在写入 store 前调用此层完成，store 层不做二次归一化，保证 store 中存储的就是最终路由键 |
...
| `internal/server/server.go` | 在最外层增加 `hostDispatchHandler`，请求顺序固定为“合法内部 WS 握手 -> HTTP 域名隧道 -> setup 例外 -> 管理面 / API / SSE -> 404”。**`securityHeadersHandler` 必须只包住管理面分支，不能包住整个外层 handler**，否则安全头（CSP / X-Frame-Options 等）会污染业务响应。`/ws/control` / `/ws/data` 只有在 path + 子协议都合法时才可进入内部通道，未带合法子协议时不能被模糊接纳。**管理面 SPA fallback 只能发生在已经命中 setup 例外或生效管理 Host 之后**，未知 Host 不能再落入 `handleWeb`。已通过子协议识别进入内部 WS 握手分支的请求，Origin 校验应豁免或单独处理，不能用管理面 Host 比对业务 Host（避免 Client 从非管理 Host 握手时被误拒）。**已知 bug（P1）**：`restoreTunnels` 中为 `paused / stopped / error` 状态以及端口不在白名单时构造的两处占位 `ProxyTunnel`，均未从 `StoredTunnel` 赋值 `Domain` 字段；重启后这些 HTTP 隧道的路由键为空字符串，导致 Host 分发失效且域名冲突扫描漏判；实现时须在两处占位构造中补全 `Domain: st.Domain`，并补对应重启恢复场景的集成测试。 |
...
| `internal/server/server_test.go` | 覆盖管理面入口、HTTP 隧道入口、内部 WS 通道三者共存的集成行为；验证合法子协议可以在非管理 Host 上建立控制 / 数据通道，非法或缺失子协议不会误入内部通道；Host 分流测试必须走最终 handler，而不是只测内部 `newHTTPMux()`。`StartHTTPOnly()` 须改为返回新的外层 handler（返回类型从 `*http.ServeMux` 改为 `http.Handler`），仓库中现有直接调用 `s.newHTTPMux()` 的测试须同步更新入口，否则测试将只覆盖内层管理面路由，Host 分流逻辑测不到。 |
...
| 测试只覆盖内部 mux | 实际最外层 Host 分流改错却测不出来；`StartHTTPOnly()` 返回旧 mux 也会成为无效覆盖 | Host 分流测试必须覆盖最终 handler；`StartHTTPOnly()` 同步改为返回新外层 handler（返回类型须从 `*http.ServeMux` 改为 `http.Handler`，这是破坏性接口变更，须同步更新所有测试调用点） |
...
2. **优先修复已确认的前置 bug**（不改就跑不通，先修再测）
   ...
   - 注：`upsertTunnelPlaceholder` 已在 `tunnel_manager.go:415` 包含 `Domain: req.Domain`，不需要修复
...
6. 再实现后端 HTTP 域名隧道最小闭环
   ...
   - `StartHTTPOnly()` 同步改为返回新外层 handler（返回类型须从 `*http.ServeMux` 改为 `http.Handler`，这是破坏性接口变更，须同步更新所有测试调用点）
