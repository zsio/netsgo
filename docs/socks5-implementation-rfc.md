...
```text
server_expose + server/socks5_listen -> client/socks5_handler
...
| 范围 | 当前根因 | 主要代码位置 | 本次处理 |
|---|---|---|---|
| DB endpoint type | SQLite `CHECK` 只允许 TCP/UDP/HTTP endpoint | `internal/server/migrations/005_unified_tunnel_storage.sql` | 新增迁移，扩展 CHECK 到 `socks5_listen` / `socks5_handler` |
| API endpoint 校验 | endpoint/topology 组合写死在 unified API 的 if-else 中 | `internal/server/unified_tunnel_api.go` 的 `validateUnifiedEndpointCombination` | 增加 `server_expose + socks5_listen -> socks5_handler`，建议抽兼容矩阵 |
...
| client capabilities | 默认 client 只声明 TCP/UDP target 能力 | `pkg/protocol/types.go` 的 `DefaultClientCapabilities` | 增加 `socks5_handler` 能力；server 对旧 client 拒绝创建/下发 |
...
| client runtime | client `handleStream` 只面向固定 TCP/UDP target | `internal/client/client.go`、`internal/client/unified_tunnel.go` | 新增 `socks5_handler` 分支，执行 target policy 与 dial |
...
TargetTypeSOCKS5Handler = "socks5_handler"
```
...
| `target.type` | `socks5_handler` |
| `transport_policy` | `server_relay_only` |

不允许：

```text
tcp_listen -> socks5_handler
client_to_client + socks5_listen -> socks5_handler
```
...
不得首期加入 `tcp_listen -> socks5_handler` 或 `client_to_client socks5_listen -> socks5_handler`。
...
`target_host` / `target_port` 对 `socks5_handler` 必填。`target_addr_type` / `original_host` 可用于保留 domain、IPv4、IPv6 语义。
...
CHECK (target_type IN ('tcp_service', 'udp_service', 'socks5_handler'))
```
...
- 新 `socks5_listen` / `socks5_handler` 可插入、读取、恢复；
- TCP tunnel 与 SOCKS5 tunnel 不能监听同一 bind ip + port；
- 两个 SOCKS5 tunnel 不能监听同一 bind ip + port。

### 13.2 协议与能力

- 未声明 `socks5_handler` 的 client 被拒绝；
- 声明 `socks5_handler` 的 client 可接收 SOCKS5 provisioning；
