...
func (s *Server) handleDataConn(conn net.Conn) {
	...
	// 6. 在该 TCP 连接上建立 yamux Server Session
	...
	// 7. 阻塞等待 session 关闭（保持连接存活）
	<-session.CloseChan()
	log.Printf("🔌 数据通道已断开: Agent [%s]", agentID)
}

// dataHandshakeBytes 构造 Agent 侧数据通道握手包
// 格式: [1B 魔数] [2B AgentID长度 big-endian] [NB AgentID]
func DataHandshakeBytes(agentID string) []byte {
