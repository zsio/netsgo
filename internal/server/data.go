...
// 此时魔数字节 (0x4E) 已被 peek 消费，连接的下一个字节是 DataToken 长度。
func (s *Server) handleDataConn(conn net.Conn) {
	...
	// 1. 读取 DataToken 长度 (2 bytes, big-endian uint16)
	var lenBuf [2]byte
	if _, err := io.ReadFull(conn, lenBuf[:]); err != nil {
		log.Printf("❌ 数据通道: 读取 Token 长度失败: %v", err)
	...
	if _, err := io.ReadFull(conn, tokenBuf); err != nil {
		log.Printf("❌ 数据通道: 读取 Token 失败: %v", err)
		conn.Write([]byte{protocol.DataHandshakeFail})
		conn.Close()
		return
	}
	...
	// 3. 通过 DataToken 查找对应的 AgentID（一次性，验证后立即删除）
	agentIDVal, ok := s.dataTokens.LoadAndDelete(dataToken)
	if !ok {
		log.Printf("❌ 数据通道: DataToken 无效或已使用")
		conn.Write([]byte{protocol.DataHandshakeFail})
		conn.Close()
		return
	}
	...
	// 7. 在该 TCP 连接上建立 yamux Server Session
	...
	// 8. 阻塞等待 session 关闭（保持连接存活）
	<-session.CloseChan()
	log.Printf("🔌 数据通道已断开: Agent [%s]", agentID)
}

// DataHandshakeBytes 构造 Agent 侧数据通道握手包
// 格式: [1B 魔数] [2B DataToken长度 big-endian] [NB DataToken]
func DataHandshakeBytes(dataToken string) []byte {
	tokenBytes := []byte(dataToken)
	buf := make([]byte, 1+2+len(tokenBytes))
